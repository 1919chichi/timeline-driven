              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {tag.max > 1 && (
                      <span className={`text-[11px] font-bold px-2 py-1 rounded-lg transition-colors ${
                        isMaxed ? "bg-white/20 text-white" : "bg-gray-200 text-gray-600"
                      }`}>
                        {count} / {tag.max}
                      </span>
                    )}
      ...
      <div className="flex justify-between items-start w-full px-1">
        <div className="min-w-0 flex-1 pr-4">
          <div className="flex items-center flex-wrap gap-2.5">
            <h3 className={`text-[16px] font-semibold tracking-tight transition-colors duration-300 ${done ? "line-through text-gray-400" : "text-gray-900"} truncate`}>
              {task.name}
            </h3>
            {infoTags.map((info, idx) => (
              <span key={`info-${idx}`} className="inline-flex items-center px-2 py-0.5 rounded-md text-[11px] font-medium bg-gray-100 text-gray-600 border border-gray-200/60 whitespace-nowrap">
                {info}
              </span>
            ))}
            {showReminder && (
